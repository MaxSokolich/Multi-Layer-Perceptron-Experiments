...
class uBotsGym(gym.Env):
    ...
    def __init__(self,
        ...
        obs_dim = 4 * self.N  # 2 bots (x,y) + 2 goals (x,y)
        self.observation_space = Box(
            low=np.array([self.XMIN, self.YMIN] * 2 + [self.XMIN, self.YMIN] * 2, dtype=np.float32),
            high=np.array([self.XMAX, self.YMAX] * 2 + [self.XMAX, self.YMAX] * 2, dtype=np.float32),
            dtype=np.float32
        )
    ...
    def _get_goal(self):
        ...
        # Fixed goal
        goal0 = np.array([10, -10.1])
        goal1 = np.array([40.1, 35.1])
    ...
    def _flatten_obs(self):
        return np.concatenate((self.positions[0],self.positions[1], np.array([self.goal0_pos, self.goal1_pos]).flatten())).astype(np.float32)
...
class uBotsGymDiscrete(uBotsGym):
    def __init__(self, N, XMIN=-10, XMAX=10, YMIN=-10, YMAX=10, dt=0.1, horizon=100, continuous_task=True, render_mode=None):
        ...
        n_actions = 4 
        n_angles = 4 # feel free to change this number for better resolution
    ...
    def step(self, action):
        # if isinstance(action, np.integer):  # Check if action is a single integer
        #     action = (int(action),)  # Convert it into a tuple
